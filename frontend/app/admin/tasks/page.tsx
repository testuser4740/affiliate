...
export default function AdminTasks() {
  ...
  const adminPendingTasks = submissions.filter(s => s.status === "Pending Review" || s.status === "Resubmitted");
          ...
          {adminPendingTasks.map(t => (
            <div key={t.submissionId} className="p-3 rounded-xl border border-[#EFEAE0] flex items-center justify-between gap-3 flex-wrap" data-testid={`pending-task-${t.submissionId}`}>
              ...
              <div className="flex gap-2">
                 <button onClick={()=>setRejecting({ pendingId: t.submissionId, ambassador: t.ambassador })} className="btn-ghost border-[#991B1B] text-[#991B1B] text-xs h-9" data-testid={`reject-${t.submissionId}`}><X className="w-3 h-3" /></button>
                 <button onClick={()=>toast.success(`${t.ambassador}'s submission approved!`)} className="btn-primary text-xs h-9" data-testid={`approve-${t.submissionId}`}><Check className="w-3 h-3" /> Approve</button>
              </div>
            </div>
          ))}
        </div>
