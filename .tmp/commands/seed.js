...
const ambassadors_1 = require("../api/models/ambassadors");
const tiers_1 = require("../api/models/tiers");
const affiliate_urls_1 = require("../api/models/affiliate-urls");
const commission_history_1 = require("../api/models/commission-history");
const activity_logs_1 = require("../api/models/activity-logs");
const tasks_1 = require("../api/models/tasks");
const task_submissions_1 = require("../api/models/task-submissions");
const payouts_1 = require("../api/models/payouts");
const referral_codes_1 = require("../api/models/referral-codes");
const announcements_1 = require("../api/models/announcements");
const pocs_1 = require("../api/models/pocs");
const inbox_messages_1 = require("../api/models/inbox-messages");
const applicants_1 = require("../api/models/applicants");
const commission_overrides_1 = require("../api/models/commission-overrides");
const order_utilization_logs_1 = require("../api/models/order-utilization-logs");
const MONTHS = {
...
const db = (0, typeormLoader_1.createDataSource)();
async function clearAll() {
    const repos = [
        order_utilization_logs_1.OrderUtilizationLog, commission_history_1.CommissionHistory, activity_logs_1.ActivityLog, affiliate_urls_1.AffiliateUrl,
        referral_codes_1.ReferralCode, task_submissions_1.TaskSubmission, tasks_1.Task, payouts_1.Payout, announcements_1.Announcement,
        pocs_1.Poc, inbox_messages_1.InboxMessage, applicants_1.Applicant, commission_overrides_1.CommissionOverride, ambassadors_1.Ambassador, tiers_1.Tier,
    ];
...
async function seed() {
    ...
    await db.getRepository(tiers_1.Tier).save(tiers.map((t) => db.getRepository(tiers_1.Tier).create(t)));
    const leaderboard = [
    ...
    const ambRepo = db.getRepository(ambassadors_1.Ambassador);
    const ambassadors = leaderboard.map((l, i) => {
        const tier = l.revenue >= 400000 ? 'Platinum' : l.revenue >= 150000 ? 'Gold' : l.revenue >= 50000 ? 'Silver' : 'Bronze';
        const commissionPct = tier === 'Platinum' ? 15 : tier === 'Gold' ? 12 : tier === 'Silver' ? 10 : 8;
        return ambRepo.create({
            ...
            affiliateLink: `https://gajab.com/r/${l.name.toUpperCase().replace(/[^A-Z]/g, '')}-${l.college.slice(0, 3).toUpperCase()}`,
            avatar: `https://images.unsplash.com/photo-${1000000000000 + i}?w=120&h=120&fit=crop`,
            tier, rank: l.rank, totalAmbassadors: 312, commissionPct, revenue: l.revenue, orders: l.orders,
        });
    });
    await ambRepo.insert(ambassadors.map((a) => ({
        id: a.id, name: a.name, college: a.college, city: a.city, state: a.state, email: a.email,
        phone: a.phone, affiliateLink: a.affiliateLink, avatar: a.avatar, tier: a.tier, rank: a.rank,
        totalAmbassadors: a.totalAmbassadors, commissionPct: a.commissionPct, revenue: a.revenue, orders: a.orders,
    })));
    ...
    const urlRepo = db.getRepository(affiliate_urls_1.AffiliateUrl);
    await urlRepo.save(adminAffiliateUrls.map((u) => urlRepo.create({
        id: u.id, ambassador: u.ambassador, college: u.college, label: u.label, url: u.url,
    ...
    const chRepo = db.getRepository(commission_history_1.CommissionHistory);
    await chRepo.save(commissionHistory.map((c) => chRepo.create({
    ...
    const alRepo = db.getRepository(activity_logs_1.ActivityLog);
    ...
    const taskRepo = db.getRepository(tasks_1.Task);
    ...
    const tsRepo = db.getRepository(task_submissions_1.TaskSubmission);
    await tsRepo.save(adminPendingTasks.map((s) => tsRepo.create({
        submissionId: s.id, ambassador: s.ambassador, college: s.college, task: s.task,
    ...
    const payoutRepo = db.getRepository(payouts_1.Payout);
    await payoutRepo.save(payouts.map((p) => payoutRepo.create(p)));
    ...
    const rcRepo = db.getRepository(referral_codes_1.ReferralCode);
    ...
    const annRepo = db.getRepository(announcements_1.Announcement);
    ...
    const pocRepo = db.getRepository(pocs_1.Poc);
    await pocRepo.save(pocList.map((p) => pocRepo.create(p)));
    const inboxMessages = [
    ...
    const inboxRepo = db.getRepository(inbox_messages_1.InboxMessage);
    await inboxRepo.save(inboxMessages.map((m) => inboxRepo.create({
        id: m.id, from: m.from, subject: m.subject, preview: m.preview, body: m.body,
        receivedOn: m.receivedOn, read: m.read, priority: m.priority,
    })));
    const applicants = [
    ...
    const appRepo = db.getRepository(applicants_1.Applicant);
    ...
    const coRepo = db.getRepository(commission_overrides_1.CommissionOverride);
    ...
    const ouRepo = db.getRepository(order_utilization_logs_1.OrderUtilizationLog);
    await ouRepo.save(referralUtilization.map((o) => ouRepo.create({
        code: o.code, orderId: o.orderId, customerId: o.customerId, orderValue: o.orderValue,
        usedAt: parseDate(o.date), discount: o.discount, commissionPct: o.commissionPct, commissionValue: o.commissionValue,
    })));
